...
for batch_index in range(BATCH_COUNT):
    ...
    data_sv_list = []
    with open('Data_SV/data_sv_processed.csv', 'r', encoding='utf-8-sig') as file:
    ...
    for data_sv in data_sv_list:
        ...
        print(f"\nReporte SRS {BATCH_COUNT}: {identification.ReportTitle}")
        ...
        if not valid_reactions:
            print(f"ERROR: No se encontró ninguna reacción válida con código MedDRA para el caso {case_number}. Omitiendo este registro.")
            continue  # Saltar al siguiente registro en lugar de detener el proceso
    ...
    # Fuera del bucle de registros, escribir todos los ICSRs del lote
    stream = FileStream(f"Output/SRS/sv_output_{batch_index+1}.xml", FileMode.Create, FileAccess.Write)
...
with open('Output/SRS/messageLog_sv.txt', 'w', newline='') as csvfile:
